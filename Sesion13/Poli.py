...
class Ingles(Ciudadano):
    def __init__(self,nombre:str,edad:int,id:int):
        super().__init__(nombre,edad)
        self.ID=id
...
class Chino(Ciudadano):
    def __init__(self,nombre:str,edad:int,sfz:int):
        super().__init__(nombre,edad)
        self.SFZ=sfz
        self.yu='Ni Hao ma?'
...
def main():
    futbolista1=Colombiano('Radamel Falcao',36,123124)
    futbolista2=Ingles('David Beckham',49,23434)
    actor=Chino('Jackie Chan',70,4564)
    Goku=Ciudadano('Son Goku',37)
    ...
    persona.setNacionalidad(input('Ingresar nacionalidad: '))
    print(persona.getNacionalidad())
